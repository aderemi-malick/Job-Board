...
##########################################################################       PEOPLES CONNECTION endpoints      #################################################################

############################ GET LOGIN #########################
@app.get("/login/{login}{passw}")
def root(login, passw):
    try:
        person = ["id_people", "lastname", "firstname", "email", "password", "status"]
        data = []
        with odbc.connect(connection_string) as conn:
            cursor = conn.cursor()
            cursor.execute(f"Select id_people, lastname, firstname, email, password, status FROM People WHERE email='{login}' AND password='{passw}' ;")
            for row in cursor.fetchall():
                result = {}
                for tag, element in (zip(person, row)):
                    result[tag] = element
                data.append(result)
            print(cursor.fetchall())
...
def root(id):
    ...
    try:
        ...
        conn = odbc.connect(connection_string)
        cursor = conn.cursor()
        cursor.execute(f"Select * FROM Ads WHERE id_people={id};")
        for row in cursor.fetchall():
            result = {}
            for tag, element in (zip(ad, row)):
                result[tag] = element
